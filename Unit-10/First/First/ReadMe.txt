================================================================================
    MICROSOFT 基础类库 : First 项目概述
===============================================================================

应用程序向导已为您创建了此 First 应用程序。此应用程序不仅演示 Microsoft 基础类的基本使用方法，还可作为您编写应用程序的起点。

本文件概要介绍组成 First 应用程序的每个文件的内容。
...
First.h
    这是应用程序的主头文件。
    其中包括其他项目特定的标头（包括 Resource.h），并声明 CFirstApp 应用程序类。

First.cpp
    这是包含应用程序类 CFirstApp 的主应用程序源文件。

First.rc
    这是程序使用的所有 Microsoft Windows 资源的列表。它包括 RES 子目录中存储的图标、位图和光标。此文件可以直接在 Microsoft Visual C++ 中进行编辑。项目资源包含在 2052 中。

res\First.ico
    这是用作应用程序图标的图标文件。此图标包括在主资源文件 First.rc 中。
...
res\Toolbar.bmp
    此位图文件用于为工具栏创建平铺图像。
    初始工具栏和状态栏在 CMainFrame 类中构造。使用资源编辑器编辑此工具栏位图，并更新 First.rc 中的 IDR_MAINFRAME TOOLBAR 数组以添加工具栏按钮。
/////////////////////////////////////////////////////////////////////////////

应用程序向导创建一种文档类型和一个视图：

FirstDoc.h、FirstDoc.cpp - 文档
    这些文件包含 CFirstDoc 类。编辑这些文件以添加特殊文档数据并实现文件保存和加载（通过 CFirstDoc::Serialize）。

FirstView.h、FirstView.cpp - 文档视图
    这些文件包含 CFirstView 类。
    CFirstView 对象用于查看 CFirstDoc 对象。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 First.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
